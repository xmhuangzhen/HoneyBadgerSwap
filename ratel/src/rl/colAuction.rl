...
contract colAuction{
    ...
    pureMpc checkFail(server, token_addr, i, colAuctionId) {
        await runCheckFailUpdate(server, token_addr, i, colAuctionId)
    }

    pureMpc checkFailUpdate(server, token_addr, i, colAuctionId) {
    ...
    pureMpc checkSuccess(server, i, colAuctionId, token_addr, curPrice, curAmt, app_token_amt) {
        curAmt,app_token_amt = await runCheckSuccessUpdate(server, i, colAuctionId, token_addr, curPrice, curAmt, app_token_amt)
        return curAmt, app_token_amt
    }

    pureMpc checkSuccessUpdate(server, i, colAuctionId, token_addr, curPrice, curAmt, app_token_amt) {
        bids = readDB(f'bidsBoard_{colAuctionId}_{i+1}', dict)
        bids = bids[i]
        vi = bids['valid']
        pricei = bids['price']
